...
def objective(trial):
    ...
    print('Training CHSJA-10: AA-TD..')
    
    eval_steps = 5000
    defense = 1 # 0 vanilla | 1 trained | 2 adaptive 
    ...
    ent_coef = 0
    radv = trial.suggest_categorical('reward', [4,6,7,8])
    ts = trial.suggest_categorical('timesteps', [6e5,1e6])
    rint = 1
    inter = 1
    ...
    print("Saving models...")
    adversary.save("mods/games/chsja10adv_" + str(trial.number) + ".pt")
    ...
    adversary = RPPO.load("mods/games/chsja10adv_" + str(trial.number) + ".pt", envv, "adversary", seed)
...
def test(num, rew):
    ...
    interceptor = RPPO.load("mods/games/chsja4int_5.pt" , envv, "interceptor", seed)
    adversary = RPPO.load("mods/games/chsja10adv_" + str(num) + ".pt", envv, "adversary", seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load', default=str("9"), type=str, help="Model to load")
