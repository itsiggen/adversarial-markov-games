...
class TestGames(gym.Env):
    ...
    def scale_delta(self, v):
        # Delta from [-2,2] to [0.0001,1.0001]
        return ((v + 2) / 4) + 0.0001
    
    def scale_step(self, v):
        # Jump step search from [-2,2] to [0.1,1.7]
        return ((v + 2) / 2.5) + 0.1
    ...
    def reset(self):
        ...
        self.chain = Chain(nrQueues=3, dataset='mnist', train=self.train)
        # Target epsilon
        self.epsilon = 1
        self.correct = []
        self.done = False
        ...
        obs, self.span = self.obs_int(self.logits, cand.raw.detach().squeeze(0).numpy(), 1)
    ...
    def step_int(self, action):
        ...
        if self.next == 0:
            ...
            obs, self.span = self.obs_int(self.logits, cand, 0)
    ...
    def phase_query(self):
        ...
        return logits, cand.raw.detach().squeeze(0).numpy()
    ...
    def phase_proceed(self):
        if self.phase == 0:
            ...
            if adv is not None:
                self.phase = 0
                self.repdone = 1
                if l2(self.wanted_point, adv.raw.squeeze(0).numpy()) < self.dist:
                    self.best_advs = adv
                self.update()
    ...
    def step_adv(self, action):
        ...
        if self.adaptive == 0 or self.adaptive == 2:
            ...
            self.action_grad = int(num_grad/5) if self.train else num_grad
    ...
    def update(self):
        distance = l2(self.wanted_point, self.best_advs.raw.squeeze(0).numpy())
    ...
    def step_ben(self, action):
        ...
        obs, self.span = self.obs_int(self.logits, candidate, 2)
    ...
    def obs_int(self, logits, query, agt):
   ...
   # Decompose steps
    ...
    def binary_proceed(self):
        if self.is_adv:
            ...
            loc = l2(self.wanted_point, self.bcand.raw.squeeze(0).numpy())
            if loc < self.bloc:
                self.imp = 1
                self.bloc = loc
    ...
    def reward3(self):
        if self.gain == 0:
            ...
            reward = -self.action_grad/500 + self.gain
        return reward
    ...
    def reward6(self):
        if self.gain == 0:
            ...
            reward = 10/self.action_grad + self.gain / self.gap
        return reward
    ...
    def swap(self, span, action):
        if action.shape == 1:
            action = action[0]
        if self.adaptive == 0:
            inn = min(span) > 0.1*self.intercept
