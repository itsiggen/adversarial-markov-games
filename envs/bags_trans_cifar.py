...
class BagsTransCIFAR(gym.Env):
    def __init__(
        ...
        super(BagsTransCIFAR, self).__init__()  
    ...
    def step_adv(self, action):
        ...
        cand = self.apply_transforms(cand, action)
    ...
    def apply_transforms(self, sample, action):
        # Low distoriton transformations as described in the stateful paper
        ...
        #         a = [1+a,1+a]
        #     else: a = action[0]
        #     trs.append(transforms.ColorJitter(brightness=a, contrast=0))
        # # contrast
        # if action[1] > 0:
        #     if self.adaptive:
        #         a = (action[1] - 1) / 2
        #         a = [1+a,1+a]
        #     else: a = [action[1], 1]
        #     trs.append(transforms.ColorJitter(brightness=0, contrast=a))
    ...
    def swap(self, span, action):
        ...
        if action.shape == 1:
            action = action[0]
        if self.defense == 0:
            inn = min(span) > 0.01*self.intercept if self.vanilla else True
        else:
            inn = min(span) > action
