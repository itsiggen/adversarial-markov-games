...
class HsjaSkip(gym.Env):
    def __init__(
        ...
        # self.dataset, model = load('MNIST', defended)
    ...
    def reset(self):
        ...
        # if self.resets < 50: print("Start:", startLabel, "| Wanted:", originLabel, "| Gap:", self.gap)
        self.dist = self.gap
        ...
        # pos = self.wanted_point.squeeze(0).numpy() - self.best_advs.squeeze(0).numpy()
        # pos = pos[::4,::4].flatten().tolist()
        
        observation = []
        observation.append(np.float32(0.0))
        observation.append(np.float32(0.5)) #meanadv
    ...
    def scale_delta(self, v):
        # Delta from [-2,2] to [0.0001,0.5001]
        base = ((v + 2) / 2) + 0.0001
    ...
    def step(self, action):
        ...
        # if self.reps % 1000 == 0:
        # fps = int(self.reps / (time.time() - self.start_time))
        # print(fps, (time.time() - self.start_time), self.reps)
        ...
        self.action_delta = 0.1*self.dist if self.reps == 1 else self.scale_delta(action[0])*self.select_delta(self.dist)
        ...
        # print(round(self.action_delta,5), round(self.action_step,3), self.action_grad, "---")
        # print(round(self.select_delta(self.dist),5), round(1/math.sqrt(self.reps),3), num_grad)
        ...
        # print(self.action_delta, self.action_step, self.action_grad, self.reps, self.iter)
