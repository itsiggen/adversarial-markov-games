...
class BoundaryStep(gym.Env):
    ...
    def step(self, actionID):
        ...
        if self.check_spherical_and_update_stats:
            ...
            self.update_stats()
            # next call is going to be source, so we return already drawn candidates
        ...
        if self.iter % (self.update_stats_every_k) == 0:
            rtr = self.spherical_candidates
        else:
            rtr = self.candidates
        obs, bIndex = self.observation(actionID, self.is_adv, rtr)
        obs = torch.nn.functional.softmax(self.logits, dim=1)
        # print("has NaNs: {}, has infs: {}".format(np.any(np.isnan(obs)), np.any(np.isinf(obs))))
        r = self.reward(self.buckets.getStepSizeBucket(bIndex), self.iter)
        # gym step returns: observation, reward, done, info
    ...
    def switch(self, actionID, candidates):
        ...
        if self.iter < 30:
            actionID = 0
        if actionID == 0:
            is_adv, self.logits = self.is_adversarial(candidates)
            self.actions[0] += 1
            self.lasta = 0
        if actionID == 1:
            is_adv, self.logits = self.mis_adversarial(candidates)
            self.actions[1] += 1
            self.lasta = 1
        # if actionID == 2:
        #     is_adv = torch.as_tensor(self.sklerion(candidates))
        #     _, self.logits = self.is_adversarial(candidates)
        #     self.actions[2] += 1
        #     self.lasta = 2
        # print(torch.nn.functional.softmax(self.logits, dim=1))
    ...
    def get_pair(self):
        startImgNr = random.randint(0,9999)
        originImgNr = random.randint(0,9999)
