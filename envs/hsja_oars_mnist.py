...
class HsjaOARSMNIST(gym.Env):
    def __init__(
        ...
        super(HsjaOARSMNIST, self).__init__()
    ...
    def reset(self):
        ...
        is_adv, output = self.is_adversarial(self.best_advs.unsqueeze(1).to(device))
        cache, detected = self.blacklight.forward(self.best_advs, output)
        if detected:
            self.det += 1
            is_adv = False
        ...
        self.var_reset()
        self.logits, cand = self.var_query()

        # Return observation for interceptor, the first agent to move
        obs, self.span = self.obs_int(self.logits, cand.detach().squeeze(0).cpu().numpy(), 1)
    ...
    def phase_query(self):
        ...
        elif self.phase == 3: logits, cand = self.var_query()
        return logits.cpu(), cand.detach().squeeze(0).cpu().numpy()
    ...
    def phase_proceed(self):
        if self.phase == 0:
            ...
            candidate, delta  = self.var_proceed()
            if delta is not None:
                self.phase = 2
                self.binary_reset(ep.astensor(candidate), self.dist)
    ...
    def step_adv(self, action):
        ...
        # ...except of delta that is adapted by OARS
    ...
    def var_query(self):
        self.var_sample += 1
        noise = torch.randn_like(self.starting_point).to(device)
        noise = noise / torch.norm(noise)
        noise = noise * self.mid
        noisy_img = self.starting_point + noise
        cand = torch.clamp(noisy_img, min=0, max=1)

        self.is_adv, self.logits = self.is_adversarial(cand.unsqueeze(0).to(device))
        return self.logits, cand

    def var_proceed(self):
        if self.is_adv: self.cache_hits += 1
        if self.var_sample == self.var_samples:
            self.var_sample = 0
            if self.cache_hits / self.var_samples <= self.hit_rate:
                self.dvar = self.mid
                self.upper = self.mid
            else:
                self.lower = self.mid
            self.var_step +=1
        # print(self.var_step, self.var_steps)
        if self.var_step == self.var_steps:
            return self.best_advs, self.dvar
        else:
            return None, None
   ...
   # Decompose steps
    ...
    def binary_query(self):
        ...
        self.is_adv, self.logits = self.is_adversarial(self.bcand.raw.unsqueeze(1).to(device))
        if self.black:
            cache, detected = self.blacklight.forward(self.bcand.raw, self.logits)
            if detected:
                self.det += 1
                self.is_adv = False
        self.logits = self.logits.cpu()
        self.iter += 1
        return self.logits, self.bcand.raw
    ...
    def grad_query(self):
        self.is_adv, self.logits = self.is_adversarial(self.perturbed[self.gsteps].raw.unsqueeze(1).to(device))
        if self.black:
            cache, detected = self.blacklight.forward(self.perturbed[self.gsteps].raw, self.logits)
            if detected:
                self.det += 1
                self.is_adv = False
        ...
        # print(self.is_adv)
        self.iter += 1
        return self.logits, self.perturbed[self.gsteps].raw
    ...
    def jump_query(self):
        ...
        self.is_adv, self.logits = self.is_adversarial(self.jcand.raw.unsqueeze(1).to(device))
        if self.black:
            cache, detected = self.blacklight.forward(self.jcand.raw, self.logits)
            if detected:
                self.det += 1
                self.is_adv = False
        # self.is_adv = self.is_adv.cpu().numpy()[0]
        ...
        self.jsteps += 1
        self.iter += 1
        return self.logits, self.jcand.raw
