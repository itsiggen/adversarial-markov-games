...
class BagsBlacklightCIFAR(gym.Env):
    def __init__(
        self,
        steps: int = 1000,
        ...
        super(BagsBlacklightCIFAR, self).__init__()  
    ...
    def step_adv(self, action):
        ...
        # sanity check
        check, _ = self.is_adversarial(self.normalize(cand).unsqueeze(0))
        ...
        self.is_adv = self.is_adv.numpy()[0] and check.numpy()[0]
    ...
    def apply_transforms(self, sample, action):
        ...
        # rotate
        if action[1] > 0:
            trs.append(transforms.RandomAffine(degrees=action[1]*5))
        # crop
        if action[2] > 0:
            a = action[2]/20
            trs.append(transforms.RandomResizedCrop(size=(32,32), scale=(0.9 - a, 0.9 + a)))
        # translate
        if action[3] > 0:
            a = action[3]/10
