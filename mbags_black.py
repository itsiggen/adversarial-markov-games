...
from envs.bags_blacklight_mnist import BagsBlacklightMNIST
...
def objective(trial):
    ...
    scale = trial.suggest_categorical('scale', [4,6,10])
    ent_coef = 0
    radv = trial.suggest_categorical('radv', [1,2,3,4,5])
    rint = 5
    ts = trial.suggest_categorical('ts', [6e5,10e5,12e5])
    
    # Create environment
    env = gym.make("BagsBlacklightMNIST-v0",
                   steps=steps,
                   ratio_benign=ratio,
                   adaptive=adaptive,
                   dataset=dataset,
    ...
    print("Saving models...")
    interceptor.save("mods/blacklight/mnistbagsint_" + str(trial.number) + ".pt")
    adversary.save("mods/blacklight/mnistbagsadv_" + str(trial.number) + ".pt")

    # Make evaluation env
    # Different seed for validation pairs.
    seed = 3
    envv = gym.make("BagsBlacklightMNIST-v0",
                    steps=eval_steps,
                    ratio_benign=ratio,
                    adaptive=adaptive,
                    dataset=dataset,
    ...
    interceptor = RPPO.load("mods/blacklight/mnistbagsint_" + str(trial.number) + ".pt", envv, "interceptor", seed)
    adversary = RPPO.load("mods/blacklight/mnistbagsadv_" + str(trial.number) + ".pt", envv, "adversary", seed)
...
def save_results(study, trial):
    with open("mbags_black_results.txt", "a") as f:
...
def test(num, rew, scale):
    ...
    interceptor = RPPO.load("mods/blacklight/mnistbagsint_" + str(num) + ".pt", envv, "interceptor", seed)
    adversary = RPPO.load("mods/blacklight/mnistbagsadv_" + str(num) + ".pt", envv, "adversary", seed)
    ...
    c = np.mean(a)
    d = np.mean(b)
    print(f'mbags_black | defended {defended}', res, c, d)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scale', default=int(6), type=int, help="Scale for step")
    parser.add_argument('--rew', default=int(5), type=int, help="Reward used")
