...
from envs.hsja_blacklight_cifar import HsjaBlacklightCIFAR
os.environ['CUDA_VISIBLE_DEVICES'] = ''
...
def objective(trial):
    ...
    radv = trial.suggest_categorical('reward', [3,7,8])
    ts = trial.suggest_categorical('ts', [6e5,10e5,12e5])
    rint = 1
    
    # Create environment
    env = gym.make("HsjaBlacklightCIFAR-v0",
                   steps=steps,
                   ratio_benign=ratio,
                   adaptive=adaptive,
                   dataset=dataset,
    ...
    print("Saving models...")
    interceptor.save("mods/blacklight/int_" + str(trial.number) + ".pt")
    adversary.save("mods/blacklight/adv_" + str(trial.number) + ".pt")

    # Make evaluation env
    # Different seed for validation pairs.
    seed = 3
    envv = gym.make("HsjaBlacklightCIFAR-v0",
                    steps=eval_steps,
                    ratio_benign=ratio,
                    adaptive=adaptive,
                    dataset=dataset,
    ...
    interceptor = RPPO.load("mods/blacklight/int_" + str(trial.number) + ".pt", envv, "interceptor", seed)
    adversary = RPPO.load("mods/blacklight/adv_" + str(trial.number) + ".pt", envv, "adversary", seed)
...
def save_results(study, trial):
    with open("cifar_black_results.txt", "a") as f:
        # Write the trial number, values, parameters, etc. to the file
...
def test(num, rew):
    ...
    interceptor = RPPO.load("mods/blacklight/int_" + str(num) + ".pt" , env, "interceptor", seed)
    adversary = RPPO.load("mods/blacklight/adv_" + str(num) + ".pt" , env, "adversary", seed)
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        study.optimize(objective, n_trials=20, gc_after_trial=True, callbacks=[save_results])
    else:
