...
from envs.hsja_oars_mnist import HsjaOARSMNIST
os.environ['CUDA_VISIBLE_DEVICES'] = ''
...
def objective(trial):
    ...
    steps = trial.suggest_categorical('steps', [800,1200,2200,3400])
    ...
    print("Saving models...")
    interceptor.save("mods/oars/mnistint_" + str(trial.number) + ".pt")
    adversary.save("mods/oars/mnistadv_" + str(trial.number) + ".pt")

    # Make evaluation env
    # Different seed for validation pairs.
    seed = 3
    envv = gym.make("HsjaOARSMNIST-v0",
                    steps=eval_steps,
                    ratio_benign=ratio,
                    adaptive=adaptive,
                    dataset=dataset,
    ...
    interceptor = RPPO.load("mods/oars/mnistint_" + str(trial.number) + ".pt", envv, "interceptor", seed)
    adversary = RPPO.load("mods/oars/mnistadv_" + str(trial.number) + ".pt", envv, "adversary", seed)
...
def save_results(study, trial):
    with open("mnist_oars_results.txt", "a") as f:
...
def test(num, rew):
    ...
    interceptor = RPPO.load("mods/oars/mnistint_" + str(num) + ".pt" , env, "interceptor", seed)
    adversary = RPPO.load("mods/oars/mnistadv_" + str(num) + ".pt" , env, "adversary", seed)
    ...
    c = np.mean(a)
    d = np.mean(b)
    print(f'mnist_oars | defended {defended}', res, c, d)
