...
from envs.bags_oars_mnist import BagsOARSMNIST
os.environ['CUDA_VISIBLE_DEVICES'] = ''
...
def objective(trial):
    ...
    radv = 1
    ts = trial.suggest_categorical('ts', [10e5,15e5])
    steps = 1000
    
    # Create environment
    env = gym.make("BagsOARSMNIST-v0",
                   steps=steps,
                   ratio_benign=ratio,
                   adaptive=adaptive,
                   dataset=dataset,
    ...
    print("Saving models...")
    interceptor.save("mods/oars/mnistbagsint_" + str(trial.number) + ".pt")
    adversary.save("mods/oars/mnistbagsadv_" + str(trial.number) + ".pt")

    # Make evaluation env
    # Different seed for validation pairs.
    seed = 3
    envv = gym.make("BagsOARSMNIST-v0",
                    steps=eval_steps,
                    ratio_benign=ratio,
                    adaptive=adaptive,
                    dataset=dataset,
    ...
    interceptor = RPPO.load("mods/oars/mnistbagsint_" + str(trial.number) + ".pt", envv, "interceptor", seed)
    adversary = RPPO.load("mods/oars/mnistbagsadv_" + str(trial.number) + ".pt", envv, "adversary", seed)
...
def save_results(study, trial):
    with open("mbags_oars_results.txt", "a") as f:
...
def test(num, rew):
    ...
    interceptor = RPPO.load("mods/oars/mnistbagsint_" + str(num) + ".pt" , env, "interceptor", seed)
    adversary = RPPO.load("mods/oars/mnistbagsadv_" + str(num) + ".pt" , env, "adversary", seed)
    ...
    print(f'mbags_oars | defended {defended}', res, c, d)
