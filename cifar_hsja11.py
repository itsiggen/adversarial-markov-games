...
from envs.hsja_trans_cifar import HsjaTransCIFAR
...
def objective(trial):
    ...
    radv = trial.suggest_categorical('radv', [7,8])
    ts = trial.suggest_categorical('ts', [6e5,12e5])

    # Create environment
    env = gym.make("HsjaTransCIFAR-v0",
                   steps=steps,
                   ratio_benign=ratio,
                   defense=defense,
                   adaptive=adaptive,
    ...
    print("Saving models...")
    interceptor.save("mods/games/chsja11int_" + str(trial.number) + ".pt")
    adversary.save("mods/games/chsja11adv_" + str(trial.number) + ".pt")
    
    # Make evaluation env
    seed = 3
    envv = gym.make("HsjaTransCIFAR-v0",
                    steps=eval_steps,
                    ratio_benign=ratio,
                    defense=defense,
                    adaptive=adaptive,
    ...
    interceptor = RPPO.load("mods/games/chsja11int_" + str(trial.number) + ".pt" , envv, "interceptor", seed)
    adversary = RPPO.load("mods/games/chsja11adv_" + str(trial.number) + ".pt" , envv, "adversary", seed)
...
def test(num, r1, r2):
    ...
    interceptor = RPPO.load("mods/games/chsja11int_" + str(num) + ".pt" , envv, "interceptor", seed)
    adversary = RPPO.load("mods/games/chsja11adv_" + str(num) + ".pt" ,  envv, "adversary", seed)
    benign = RandomAgent(env=envv)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--r1', default=int(6), type=bool, help="R1 used")
