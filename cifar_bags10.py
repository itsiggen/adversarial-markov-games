...
def objective(trial):
    ...
    print("Saving models...")
    adversary.save("mods/games/cbags10adv_" + str(trial.number) + ".pt")
    ...
    adversary = RPPO.load("mods/games/cbags10adv_" + str(trial.number) + ".pt" , envv, "adversary", seed)
                
    benign = RandomAgent(env=envv)

    mean_rint, std_rint, mean_radv, std_radv, epsilons, lengths, mean_eps, start_eps, mean_acc = evaluate_rpolicy(interceptor, adversary, benign, envv, n_eval_episodes=25)
...
def test(num, rew):
    ...
    interceptor = RPPO.load("mods/games/cbags4int_2.pt" , envv, "interceptor", seed)
    adversary = RPPO.load("mods/games/cbags10adv_" + str(num) + ".pt", envv, "adversary", seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load', default=str("17"), type=str, help="Model to load")
